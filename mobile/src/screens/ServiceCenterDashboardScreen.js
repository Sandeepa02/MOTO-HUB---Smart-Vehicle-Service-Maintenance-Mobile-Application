...
const actions = [
  { label: 'Incoming Bookings', screen: 'ServiceCenterBookings' },
  { label: 'Maintenance Records', screen: 'Maintenance' },
  { label: 'My Service Center', screen: 'ServiceCenterProfile' }
];

export default function ServiceCenterDashboardScreen({ navigation }) {
  const { user, logout } = useAuth();

  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>Service Center Dashboard</Text>
      <Text style={styles.subtitle}>{user?.serviceCenterProfile?.centerName || user?.name || 'Service Center'}</Text>
      {actions.map((action) => (
